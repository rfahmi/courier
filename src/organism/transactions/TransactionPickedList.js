...
const TransactionPickedList = ({status, option}) => {
  ...
  const getData = async (p) => {
    ...
    const append_url = option ? `&option=${option}` : '';
    const result = await api
      .get(
        `/courier/${courier_data.kurir_id}/transaction/picked?page=${p}&limit=${limit}&status=${status}${append_url}`,
        {
          headers: {
...
export default memo(TransactionPickedList);
